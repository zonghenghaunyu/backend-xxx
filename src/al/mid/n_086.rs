...
mod tset {
    ...
    fn partition001(s : String) -> Vec<Vec<String>>{
        ...
        inner_partition001(&is_right,&arr, 0, arr.len() - 1, &mut cache, &mut ans, arr.len());

        println!("{:?}",ans);
        ans
    }

    fn inner_partition001(is_right : &HashSet<(usize,usize)>,arr : &Vec<char>,start: usize,end: usize,cache : &mut Vec<String>,ans: &mut Vec<Vec<String>>,len : usize){
        if start >= len{
